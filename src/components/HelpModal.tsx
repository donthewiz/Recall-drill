        ...
        <div className="space-y-3.5 text-xs text-[var(--text-secondary)] leading-relaxed">
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p>
                Cards are encoded a batch at a time (5 by default, adjustable from 3-10, or the whole deck at once) instead of one card being drilled to completion before the next begins — within a batch you rotate to the next unfinished card after every correct rep, so repeated attempts on the same thing are spread out instead of run back to back. A long answer split into parts is the exception: it stays on screen while its parts are learned, combined, and any weak spot is reinforced, and rotation starts once you&rsquo;re recalling the whole answer. Once a batch is fully encoded you get a summary and a choice to continue or stop before the next batch. From there, encoded cards enter a review cycle — shuffled by default, or in deck order if you choose &ldquo;In order&rdquo; — and a card is marked <strong>Mastered</strong> once you recall it correctly twice, with the rest of the pass in between (the exception is when only one card in a batch&rsquo;s cycle is left to master, which can still come straight back). Once every batch is done, a <strong>Final check</strong> runs once over the whole deck: every card, shuffled, with no cues, each needing exactly one more correct answer — including that last card — before the session finishes.
              </p>
            </div>
          </div>
